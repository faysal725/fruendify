...
const selectOption = (option) => {
  ...
  emits("update:modelValue", amountOfTime.value);
  searchTerm.value = "";
  showOptionPanel.value = false;
  emits("select", option);
};
  ...
  <div>
    ...
    <section class="flex items-center gap-3">
      <input
        ...
        class="bg-transparent text-gray-700 dark:text-slate-300 h-10 text-center border border-gray-300 placeholder:text-gray-700 focus:ring-1 focus:ring-primary text-xs font-base focus:outline-none capitalize rounded-md px-4 text-start"
        type="number"
      />
      <div class="relative dropdown-container">
        <input
          ...
          :class="[
            props.disabled
              ...
              : 'bg-transparent  text-gray-700 dark:text-slate-300',
            'w-full h-10 flex items-center text-center  px-4  border border-gray-300 cursor-pointer placeholder:text-gray-700  focus:ring-1 focus:ring-primary text-xs font-base  focus:outline-none capitalize text-start',
            classes,
          ]"
          ref="searchInput"
        />
        ...
        <ChevronDownIcon
          v-else
          class="h-4 w-4 absolute top-2 right-3 transition-all duration-500 cursor-pointer"
          :class="[showOptionPanel && 'rotate-180', iconClasses]"
        />

        <!-- <ClockIcon
        class="h-4 w-4 absolute top-6 left-3 transition-all duration-500 cursor-pointer text-slate-500"
      /> -->
        <transition
          mode="out-in"
