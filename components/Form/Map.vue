...
function addingListChields(data) {
  ...
  data.map((placeData) => {
    ...
    // console.log(placeData.properties.display_name)
    // console.log(placeData.html)
    listItem.innerHTML = `<span class="leaflet-control-geocoder-address-context">
      
      ${placeData.properties.display_name}</span>
`;
    ...
    listItem.onclick = function () {
      zoom.value = 5;
      console.log(placeData.properties, "this");
      ...
      userMarker.value.neighbourhood = placeData.properties.address
        .neighbourhood
        ? placeData.properties.address.neighbourhood
        : placeData.properties.address.road;
...
onMounted(() => {
  if (props.oldValue.latitude !== 0 && props.oldValue.longitude !== 0) {
    setTimeout(() => {
      ...
      userMarker.value.road = props.oldValue.road;
      userMarker.value.neighbourhood = props.oldValue.neighbourhood;
...
// this code is responsible for mobile device bug
// watch(
...
<template>
  <div>
    ...
    {{ coords.longitude }} -->
      ...
      <div :id="id" :class="classes" class="z-0"></div>

      <div class="absolute top-3 left-0 z-10">
        <div
          class="ml-2 h-5 w-5 animate-spin rounded-full border-4 border-solid border-current border-e-transparent border-primary align-[-0.125em] text-surface motion-reduce:animate-[spin_1.5s_linear_infinite] dark:text-white"
          :class="loaderState ? 'inline-block' : 'hidden'"
          role="status"
        ></div>
      </div>
    </div>
