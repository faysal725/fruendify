...
watch(
  () => date.value,
  () => {
    console.log(date.value, 'first');

    if (!dateOverMin.value) {
      emit("update:modelValue", "");
    } else {
      emit("update:modelValue", date.value.substring(0, 10));
    }
