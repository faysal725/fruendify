...
const sendJoinRequest = async () => {
  ...
  await crud
    .store({
      body: "",
      path: joinRequestApi + eventDetailsStore.currentDetails._id,
    })
    .then((res) => {
      if (res.status == 200) {
  ...
  >
    ...
    >
      <div class="space-y-5 relative cursor-default col-span-12 lg:col-span-7">
        <ContainerEventImg2 :enableEditBtn="false" />
        ...
        >
          ...
          >
            <div
              class="flex items-center gap-3 w-full  justify-start"
            >
              ...
              <div class="flex items-center gap-3 ml-auto">
                <button
                  ...
                  v-if="
                    !eventDetailsStore.currentDetails?.joinStatus &&
                    !isOwnersProfile(
                      eventDetailsStore.currentDetails?.user_uid?._id
                    )
                  "
                ...
                <button
                v-if="eventDetailsStore.currentDetails?.joinStatus && eventDetailsStore.currentDetails?.joinStatus !== 'APPROVED'" 
                  @click="sendJoinRequest()"
      ...
      <div class="col-span-12 lg:col-span-5">
        <ContainerEventParticipantsCommon />
      </div>
    </div>
