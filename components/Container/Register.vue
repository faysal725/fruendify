...
const collectNameAndBirthday = () => {
  ...
  if (
    ...
    if (form.value.last_name === "") {
      errors.value.last_name = "Please enter your last name";
    }

    if (form.value.birth_date === "") {
      errors.value.birth_date = "You must be 18 years of age or older";
    }
  }
};
...
async function convertToFormData() {
  ...
  formData.append(
    "registered_token",
    authStore.userData.registered_token
      ? authStore.userData.registered_token
      : authStore.userDataAfterLogin.registered_token
  );
  formData.append(
    "full_name",
    authStore.userData.full_name + " " + authStore.userData.last_name
  );
  ...
  for (const interest of authStore.userData.interests) {
...
const getProfileData = async () => {
  ...
  await crud.index({ path: profileDataApi }).then(async (res) => {
    if (res.status == 200) {
      ...
      // console.log(email, first_name, birth_date, gender, "inside");
      await saveAuthProfileInfo(res.res.data);
      await saveProfileData(res.res.data);
      await sendCoords();
      await setLoaderState("resolved");
...
const getProfileDataFromGoogle = async () => {
  setLoaderState("pending");
  await crud.index({ path: profileDataApi }).then(async (res) => {
    if (res.status == 200) {
      let { email, first_name, birth_date, about_fruendify, gender } =
      ...
      if (email && first_name && birth_date && about_fruendify && gender) {
        await saveAuthProfileInfo(res.res.data);
        await saveProfileData(res.res.data);
        await sendCoords();
        await setLoaderState("resolved");
  ...
  await crud
    ...
    .then(async (res) => {
      ...
      if (res.status == 200) {
        ...
        showToast("Registration Failed. Please Fill the form properly");
        loaderState.value = "failed";
        setStep(3);
      }
    });
...
const googleCallback = async (data) => {
  ...
  await crud
    ...
    .then(async (res) => {
      ...
      if (res.status == 200) {
        if (res.res.data.registered_token) {
          open();
          await authStore.savePrimaryData(res.res);
          loaderState.value = "resolve";
          setStep(3);
        } else if (res.res.data.tokens.access) {
          authStore.saveAuthAccess(res.res.data);
          await getProfileData();
        }

        // await savePrimaryOauthData(tokens);

        // await saveOauthToken(tokens);

        // await getProfileDataFromGoogle();
...
if (route.query.state && route.query.code && authStore.isOauthCalling) {
  setIsOAuthCalling(false);
...
watch(
  () => form.value,
  () => {
    if (form.value.joining_reason === "others") {
    ...
    >
      ...
      <section v-if="stepNo == 1" class="p-5 bg-whitePrimary space-y-8">
        <div class="flex justify-center flex-col items-start gap-y-2">
          <p class="text-3xl font-semibold text-mediumBlack">
            Create an account
          </p>
        </div>
      ...
      <section v-if="stepNo == 3" class="p-5 bg-whitePrimary space-y-2">
        <div class="flex justify-center flex-col items-start gap-y-2">
        ...
        <div
          class="mt-4 space-y-8 max-h-[50vh] overflow-y-scroll scrollbar-hide"
        >
          <div class="mb-2">
            <FormTextArea
        ...
        <div class="text-center space-y-1 flex justify-end">
          <div class="w-28">
            <ButtonLarge
              @click="collectAboutFrundifyData()"
              :name="'Continue'"
            />
          </div>
        </div>
      ...
      <section v-if="stepNo == 4" class="p-5 bg-whitePrimary space-y-5">
        ...
        <div class="max-w-md">
          ...
          >
            ...
            you use our mobile application. Please read this Privacy Policy
      ...
      <section v-if="stepNo == 5">
        ...
        <div class="grid grid-cols-4 gap-4 gap-x-3 py-3 px-5">
          ...
          <div class="col-span-4">
            <FormInputText2
              ...
              :oldValue="authStore.userData.last_name"
            />
          </div>
        ...
        <div>
          <!--<FormDate
            ...
            :error="errors.birth_date"
            :oldValue="authStore.userData.birth_date"
            :errorMessage="'You have to be more then 18 years older.'"
          />-->
          <!--
          <FormDate
            v-model="form.birth_date"
      ...
      <section
        v-if="stepNo == 6"
        class="p-1 md:p-5 bg-whitePrimary space-y-2 md:space-y-5"
      >
        ...
        <div class="flex justify-between items-center pt-8 px-2 md:px-5">
          <ArrowLeftIcon
            @click="stepNo = 4"
            class="h-6 w-6 text-mediumBlack dark:text-slate-200 cursor-pointer"
          />
      ...
      <section
        v-if="stepNo == 7"
        class="p-5 bg-whitePrimary space-y-5 max-h-[80vh] overflow-y-scroll scrollbar-hide"
      >
        <div class="flex justify-center flex-col items-start gap-y-4">
          <CommonDotPaginator :stepNo="stepNo - 4" :total="5" />
          <div class="space-y-4">
            ...
            <p class="text-sm md:text-lg font-base text-gray-600">
              ...
              your career information helps us tailor your experience and
        ...
        <div class="flex justify-between items-center pt-8 md:px-5">
          <ArrowLeftIcon
            @click="stepDown()"
            class="h-6 w-6 text-mediumBlack dark:text-slate-200 cursor-pointer"
          />
      ...
      <section
        v-if="stepNo == 8"
        class="p-5 bg-whitePrimary space-y-2 md:space-y-5"
      >
        <div class="flex justify-center flex-col items-start gap-y-4">
          <CommonDotPaginator :stepNo="stepNo - 4" :total="5" />
        ...
        <div class="max-h-36 overflow-y-scroll md:max-h-full">
          <div class="pb-3">
            <FormSelector5
              ...
              :uniqueContainerClass="'Interests-register'"
              :shouldPanelOpen="true"
            />
          </div>
        </div>

        <div class="flex justify-between items-center pt-4 md:pt-8 px-5">
          <ArrowLeftIcon
            @click="stepDown()"
            class="h-4 md:h-6 w-4 md:w-6 text-mediumBlack dark:text-slate-200 cursor-pointer"
          />
          <div class="w-20 md:w-36">
      ...
      <section v-if="stepNo == 9" class="px-4 pb-3 md:min-w-96">
        <div class="flex justify-center flex-col items-start gap-y-4 py-5">
          ...
          <div class="space-y-3">
            <p
              class="text-xl md:text-3xl font-bold text-start text-mediumBlack"
            >
              Show off your style!
            </p>
